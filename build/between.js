  ...
  */
  ...
  function (_Events) {
    ...
    function Between(startValue, destValue) {
      ...
      var middleware = _this.middleware = Object.values(Between._middleware).reduce(function (v, m) {
        return v || m && m.test && m.test(startValue) && m;
      }, false);
      var type = middleware && middleware.name || (_typeof(startValue) === 'object' ? Array.isArray(startValue) ? 'array' : 'object' : 'number');
      var result = middleware.initialize(startValue, destValue);
      startValue = result.startValue;
      destValue = result.destValue;
      _this.data = result.data; // const toInterpolate = this.middleware.interpolate(startValue, destValue, Math.min(1, 0 / 1000), data)
  ...
  Between._middleware = {}; // middleware code
